...
include 'db_connect.php'; // ensure $conn is defined
...
if(isset($_SESSION['student'])){
    ...
    if(isset($_FILES['profile_picture']) && $_FILES['profile_picture']['name'] != ""){
        ...
        if(!in_array($file_ext, $allowed_ext)){
            ...
            $target_file = $target_dir . $student_id . "_" . time() . "." . $file_ext;
            $check = getimagesize($_FILES["profile_picture"]["tmp_name"]);
            if($check !== false){
                if(move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file)){
                    $stmt = $conn->prepare("UPDATE student_register SET profile_picture = ? WHERE student_id = ?");
                    $stmt->bind_param("si", $target_file, $student_id);
                    if($stmt->execute()){
                        $upload_message = "Profile picture updated successfully!";
                        $student['profile_picture'] = $target_file;
                    } else {
    ...
    $profile_pic = !empty($student['profile_picture']) && file_exists($student['profile_picture']) 
                    ? $student['profile_picture'] 
                    : "profiles/default.png";

    // Fetch student subjects
    $stmt = $conn->prepare("SELECT subject, subject_group FROM students_subject_group WHERE student_id = ?");
    $stmt->bind_param("i", $student_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
.subject-card h3{color:var(--primary);margin-bottom:15px;}
.subject-item{
    padding:10px 15px;background:rgba(255,255,255,0.05);
...
.subject-item:hover{background:rgba(0,212,255,0.1);box-shadow:0 0 15px var(--primary)20;}
...
/* Buttons */
button{
    background: linear-gradient(45deg,#4dabf7,#00d4ff);
    border:none;padding:12px 25px;border-radius:12px;color:#fff;font-weight:500;
...
<div class="container">
    ...
    <div class="content">
        <?php if(isset($_GET['page']) && $_GET['page']=='profile'): ?>
            ...
            <div class="profile-card">
                ...
                <div class="profile-right">
                    <div class="info-row"><i class="fa fa-id-badge"></i> <span>ID: <?php echo $student['student_id'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-address-card"></i> <span>NIC: <?php echo $student['nic_number'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-phone"></i> <span>Contact: <?php echo $student['contact_number'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-map-marker-alt"></i> <span>Address: <?php echo $student['address'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-school"></i> <span>School: <?php echo $student['school'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-user-friends"></i> <span>Parent: <?php echo $student['parent_name'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-phone-square"></i> <span>Parent Contact: <?php echo $student['parent_contactnumber'] ?? '-'; ?></span></div>
                    <div class="info-row"><i class="fa fa-calendar-alt"></i> <span>Exam Year: <?php echo $student['exam_year'] ?? '-'; ?></span></div>
                </div>
            </div>
                ...
                <?php if(!empty($subjects)): ?>
                    <?php foreach($subjects as $sub): ?>
                        <div class="subject-item">
                            ...
                            <span><?php echo htmlspecialchars($sub['subject_group']); ?></span>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
        ...
        <?php else: ?>
            <h1>Welcome, <?php echo $student['name'] ?? "Student"; ?>!</h1>
            <p>Use the menu to view your profile and subjects.</p>
        <?php endif; ?>
    </div>
</div>
</body>
